...
function buildHtml(client_name: string, package_type: string, dateStr: string, booking_id: string) {
  return `<!DOCTYPE html>
<html lang="vi">
      ...
      <div style="background:#f8fafc;border-radius:12px;padding:20px;margin-bottom:24px">
        ...
        <table style="width:100%;border-collapse:collapse">
          ...
          <tr>
            ...
            <td style="padding:6px 0;font-size:13px;color:#265C59;font-weight:700;font-family:monospace">#${booking_id.slice(0, 8).toUpperCase()}</td>
          </tr>
        </table>
      </div>
...
export async function POST(request: NextRequest) {
  ...
  const html = buildHtml(client_name ?? '', package_type ?? '', dateStr, booking_id ?? '');
  const subject = `✅ Xác nhận đặt lịch - ${package_type || 'Tour'} | Cao Bằng Eco Tour`;
  ...
  if (gmailUser && gmailPass) {
    ...
    await transporter.sendMail({
      from: `"Cao Bằng Eco Tour" <${gmailUser}>`,
      to,
      subject,
      html,
    });
  ...
  if (resendKey) {
    ...
    const res = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({ from: `Cao Bằng Eco Tour <${fromEmail}>`, to: [to], subject, html }),
    });
    if (!res.ok) {
